...
class ARTrainer(object):
    ...
    def eval_ep(self, ld_val: DataLoader):
        ...
        for obj in ld_val:
            ...
            if self.ar_wo_ddp.obs_encoder is not None: # Image-Based
                nobs: List[FTen] = self.ar_norm.normalize(obj['obs']) # [obs,...]
            else: # State-Based
                nobs: List[FTen] = self.ar_norm['obs'].normalize(obj['obs']) # [obs,...]
            ...
            logits_BLV = self.ar_wo_ddp(nobs, x_BLCv_wo_first_l)
            ### 
    ...
    def train_step(
        ...
        if self.ar_wo_ddp.obs_encoder is not None: # Image-Based
            ...
            nobs: List[FTen] = self.ar_norm['obs'].normalize(nobs) # [obs,...]
        ### params
        ...
        with self.ar_opt.amp_ctx:
            ...
            logits_BLV = self.ar_w_ddp(nobs, x_BLCv_wo_first_l) # transformer inference | BLV | multi-task
            ## loss
