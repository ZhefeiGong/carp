...
def build_everything(args: arg_util.Args):
    ...
    if 'kitchen' in args.data_name:
        ...
        dataset_train, dataset_val, normalizer = load_robomimic_image_dataset(data_path=args.data_path, 
            ...
            if dist.is_local_master():
                ...
                if args.is_rollout_during_train:
                    ...
                    step_log = env_runner.run(vae=trainer.vae_local, 
                                              ...
                                              normalizer=trainer.ar_norm) # for multi-task
                    metric_dict = dict()
                    for key, value in step_log.items():
                        new_key = key.replace('/', '_')
                        metric_dict[new_key] = value
                    test_mean_score = metric_dict['test_mean_score']
                    train_mean_score = metric_dict['train_mean_score']
                    local_out_json = os.path.join(args.runner_out_dir_path, f'ep{ep+1}_eval_log.json')
                ...
                if args.is_rollout_during_train: # top k according to the rollout results
                    current_ckpt = (local_out_ckpt, local_out_json, test_mean_score, train_mean_score)
                    ...
                    if current_ckpt in temp_ckpts:
                        ...
                        for worst_ckpt, worst_json, _, _ in removed_ckpts:
                            if os.path.exists(worst_ckpt): # remove the corresponding ckpt
                                os.remove(worst_ckpt)
                            if os.path.exists(worst_json): # remove the corresponding json
                                os.remove(worst_json)
                    else:
                        # delete the json saved before when current ckpt is not good enough
                        if os.path.exists(local_out_json): # remove the json
                            os.remove(local_out_json)
