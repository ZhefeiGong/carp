...
def _change_builtin_print(is_master):
    """
    @func : 
    modify the behavior of the print function 
    so that output can be disabled when it is not the main process, 
    preventing multiple processes from printing logs simultaneously.
    """
...
class SyncPrint(object):
    """
    @func: 
    output log files and terminals at the same time
    """

    def __init__(self, local_output_dir, sync_stdout=True):
        """
        @func: 
        initializes the output file path and creates or appends log files
        """
    ...
    def write(self, message):
        """
        @func: 
        write logs to both files and terminals
        """
    ...
    def flush(self):
        """
        @func: 
        force data from the buffer to be written to a file
        """
...
class DistLogger(object):
    """
    @func: 
    control the log output level and determine whether to record logs based on the verbose parameter
    """
...
class TensorboardLogger(object):
    """
    @func: 
    use TensorBoard to record various metrics during training
    """
...
class SmoothedValue(object):
    """
    @func : 
    track a series of values and provide access to smoothed values over a window or the global series average, and
    it also provides smooth statistical values such as median, average, maximum, etc
    """
...
class MetricLogger(object):
    """
    @func: 
    track and record metrics in each iteration
    """
    ...
    def log_every(self, 
        ...
        """
        @func: 
        load the data obj for every training step
        """
        ...
        if isinstance(itrt, Iterator) and not hasattr(itrt, 'preload') and not hasattr(itrt, 'set_epoch'):
            for i in range(start_it, max_iters):
                obj = next(itrt)    # "generate" obj
                self.data_time.update(time.time() - self.iter_end_t)
                yield i, obj    # "return" obj and "wait" for the next iteration of the parent process
                ...
                if i in self.log_iters:
                    ...
                    print(log_msg.format(
                        ...
                        data=str(self.data_time)), flush=True)
                self.iter_end_t = time.time()
        else:
        ...
        print('{}   Total time:      {}   ({:.3f} s / it)'.format(header, total_time_str, total_time / max_iters), flush=True)
...
def create_npz_from_sample_folder(sample_folder: str):
    """
    @func: 
    build a single .npz file from a folder of .png samples. Refer to DiT.
    """
    import os, glob
    import numpy as np
