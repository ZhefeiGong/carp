...
import torch
import numpy as np
from env.dataset.robomimic_replay_lowdim_dataset import RobomimicReplayLowdimDataset, normalizer_from_stat
...
class MultiLowdimDatasetWrapper(torch.utils.data.Dataset):
    def __init__(self, datasets: Dict[str, RobomimicReplayLowdimDataset]):
        """
        @func: 
        """
    ...
    def __getitem__(self, idx):
        ...
        data['task'] = torch.tensor(dataset_idx, dtype=torch.long) # task condition | size [B] for each iter
        return data
    ...
    def get_normalizer(self, is_only_act=False, **kwargs) -> LinearNormalizer:
        """
        @func:
        """     
        # init
        ...
        if is_only_act is False:
            obs_stat = array_to_stats(self.combine_replay_buffer('obs'))
