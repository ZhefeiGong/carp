...
# from roboenv.sync_vector_env import SyncVectorEnv
...
# from roboenv.base_lowdim_policy import BaseLowdimPolicy
...
class RobomimicLowdimRunner(BaseLowdimRunner):
    ...
    def __init__(self, 
        ...
        if abs_action:
            ...
            rotation_transformer = RotationTransformer('axis_angle', 'rotation_6d') # for diffusion policy | 6d rotation representation
        control_delta = env_meta['env_kwargs']['controller_configs']['control_delta']
        print(f'[INFO] the control delta is set to : {control_delta}')
    ...
    def run(self, 
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                env_action = action
                if self.abs_action:
                    env_action = self.undo_transform_action(action) # for diffusion policy | 6d rotation representation
